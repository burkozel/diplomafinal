...
class Uploadsmt(APIView):
    def post(self, request, *args, **kwargs):
        if not request.user.is_authenticated:
            return JsonResponse({'Status': False, 'Error': 'Log in required'}, status=403)
        if request.user.type != 'shop':
            return JsonResponse({'Status': False, 'Error': 'Только для магазинов'}, status=403)
        data = yaml.load("shop1.yaml", Loader=Loader)
        ...
        for good in data['goods']:
            ...
            product_info = ProductInfo.objects.create(product_id=product.id, model=good['model'], external_id=good['id'], shop_id=shop.id, price=good['price'], price_rrc=good['price_rrc'], quantity=good['quantity'])
